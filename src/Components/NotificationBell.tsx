...
const NotificationBell: React.FC = () => {
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className={`relative p-2 rounded-lg text-slate-800 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition ${shake ? "animate-[wiggle_0.5s_ease-in-out]" : ""}`}
        title="Notifications"
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] flex items-center justify-center text-[10px] font-bold text-white bg-slate-700 rounded-full px-1 leading-none">
            ...
            <div className="flex items-center gap-1">
              {unreadCount > 0 && (
                <button onClick={markAllAsRead}
                  className="text-sm font-medium text-slate-800 dark:text-slate-400 hover:underline font-medium px-2 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                  Mark all read
                </button>
              )}
              {notifications.length > 0 && (
                <button onClick={clearAll}
                  className="text-sm font-medium text-slate-800 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 px-2 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                  Clear all
                </button>
              )}
            </div>
