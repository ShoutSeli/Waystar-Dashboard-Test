...
const EligibilityCheck: React.FC = () => {
  ...
  const statusConfig: Record<string, { pill: string; dot: string; label: string }> = {
    Eligible:       { pill: "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300", dot: "bg-emerald-500", label: "Eligible" },
    "Not Eligible": { pill: "bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300",                dot: "bg-red-500",     label: "Not Eligible" },
    "Checking...":  { pill: "bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300",         dot: "bg-amber-400 animate-pulse", label: "Checking…" },
    Pending:        { pill: "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300",               dot: "bg-gray-400",    label: "Pending" },
  };
  ...
  return (
    <Layout currentPage="Eligibility Check">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 m-4 sm:m-6 lg:m-10">
        {[
          { label: "Eligible",           value: eligibleCount,    color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-900/20", icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" },
          { label: "Not Eligible",       value: notEligibleCount, color: "text-red-600",     bg: "bg-red-50 dark:bg-red-900/20",         icon: "M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" },
          { label: "Pending / Checking", value: pendingCount,     color: "text-amber-600",   bg: "bg-amber-50 dark:bg-amber-900/20",     icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" },
          ...
          <div key={label} className="bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 rounded-xl p-4 flex items-center gap-4">
            <div className={`${bg} p-2.5 rounded-lg shrink-0`}>
              <svg className={`w-5 h-5 ${color}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d={icon} /></svg>
            </div>
            <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mx-4 sm:mx-6 lg:mx-10 mb-4">
        <p className="text-sm text-gray-500 dark:text-gray-400">
          {patients.length} patients · tap <span className="font-medium text-gray-700 dark:text-gray-200">Check</span> to verify eligibility
        </p>
        <button onClick={checkAll} disabled={pendingCount === 0}
          ...
          <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
          Check All Pending
        </button>
      </div>
      ...
      <div className="hidden sm:block bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden mx-4 sm:mx-6 lg:mx-10">
                  ...
                  <React.Fragment key={patient.id}>
                    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300">{patient.id}</span>
                      </td>
                      <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">{patient.name}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 flex-wrap">
                          <button onClick={() => setExpandedRow(expandedRow === patient.id ? null : patient.id)} title="View details"
                          ...
                          <button onClick={() => openEdit(patient)} title="Edit"
                          ...
                          <button onClick={() => setDeleteConfirmId(patient.id)} title="Delete"
                          ...
                          {isDone
                            ? <span className="flex items-center gap-1 text-xs text-gray-400 dark:text-gray-500 ml-1">
                                <svg className="w-4 h-4 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>Verified
                              </span>
                            : <button onClick={() => sendCheck(patient.id)} disabled={isChecking}
                                className="flex items-center gap-1.5 px-3 py-1.5 bg-color-1 text-white rounded-lg text-xs font-semibold hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed ml-1">
                                {isChecking
        ...
        <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 text-xs text-gray-400 dark:text-gray-500">{patients.length} total patients</div>
      </div>
      ...
      <div className="sm:hidden grid grid-cols-1 gap-3 mx-4 mb-6">
        {patients.map(patient => {
          ...
          const isDone = patient.status === "Eligible" || patient.status === "Not Eligible";
          return (
            <div key={patient.id} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 space-y-3">
              ...
              <p className="font-medium text-gray-800 dark:text-gray-100">{patient.name}</p>
              <div className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                {isDone
                  ...
                  : <button onClick={() => sendCheck(patient.id)} disabled={isChecking}
                      className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-color-1 text-white rounded-lg text-xs font-semibold hover:bg-blue-700 transition disabled:opacity-50">
                      {isChecking
                        ? <><svg className="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Checking…</>
                        : "Check Eligibility"}
                    </button>}
                <button onClick={() => openEdit(patient)} className="p-2 rounded-lg text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </button>
                <button onClick={() => setDeleteConfirmId(patient.id)} className="p-2 rounded-lg text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-gray-200 dark:border-gray-700">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-1">Delete Patient Record</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400">Are you sure you want to delete <span className="font-medium text-gray-700 dark:text-gray-200">{deleteConfirmId}</span>? This cannot be undone.</p>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-3">
                <div className="bg-blue-100 dark:bg-blue-900/40 p-2 rounded-lg">
                  <svg className="w-5 h-5 text-blue-600 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </div>
                <div><h2 className="text-base font-semibold text-gray-800 dark:text-gray-100">Edit Patient</h2><p className="text-xs text-gray-400 dark:text-gray-500">{editingPatient.id}</p></div>
              </div>
              <button onClick={() => setEditingPatient(null)} className="p-1.5 rounded-lg text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
            ...
            <div className="px-6 py-5">
              ...
              <input type="text" value={editName} onChange={e => setEditName(e.target.value)}
                className="mt-1 w-full px-3 py-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:bg-white dark:focus:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 transition" />
            </div>
            <div className="flex gap-2 px-6 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 rounded-b-2xl">
              <button onClick={() => setEditingPatient(null)} className="flex-1 px-4 py-2 text-sm text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition font-medium">Cancel</button>
