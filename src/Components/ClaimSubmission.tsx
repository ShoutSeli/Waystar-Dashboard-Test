...
const ClaimSubmission: React.FC = () => {
  ...
  const summaryCards = [
    { label: "Total Billed", value: `$${totalAmount.toLocaleString()}`, icon: "M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" },
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
                {["Claim ID", "Patient", "Amount", "Status", "Actions"].map(h => <th key={h} className={h === "Actions" ? `${TH} translate-x-20` : TH}>{h}</th>)}
              </tr>
            </thead>
                      ...
                      <div className="flex items-center gap-1 translate-x-20">
                        ...
                        {claim.status === "Pending"
                          ? <button title="Review and submit" onClick={() => setSelectedClaim(claim)}
                              className={`translate-x-8 ml-8 mr-12 ${BTN_PRIMARY} py-1.5 text-sm`}>
