...
interface FieldProps {
  ...
  type?: string; datalist?: string[]; placeholder?: string;
  formData: PayerView; formErrors: Partial<Record<keyof PayerView, string>>;
...
const InsurancePayerView: React.FC = () => {
  ...
  const summaryCards = [
    ...
    { label: "Unique Insurers", value: uniqueInsurers, color: "#f97316", icon: <><path fillRule="evenodd" d="M4.5 2.25a.75.75 0 000 1.5v16.5h-.75a.75.75 0 000 1.5h16.5a.75.75 0 000-1.5h-.75V3.75a.75.75 0 000-1.5h-15zM9 6a.75.75 0 000 1.5h1.5a.75.75 0 000-1.5H9zm-.75 3.75A.75.75 0 019 9h1.5a.75.75 0 010 1.5H9a.75.75 0 01-.75-.75zM9 12a.75.75 0 000 1.5h1.5a.75.75 0 000-1.5H9zm3.75-5.25A.75.75 0 0113.5 6H15a.75.75 0 010 1.5h-1.5a.75.75 0 01-.75-.75zM13.5 9a.75.75 0 000 1.5H15A.75.75 0 0015 9h-1.5zm-.75 3.75a.75.75 0 01.75-.75H15a.75.75 0 010 1.5h-1.5a.75.75 0 01-.75-.75zM9 19.5v-2.25a.75.75 0 01.75-.75h4.5a.75.75 0 01.75.75v2.25a.75.75 0 01-.75.75h-4.5A.75.75 0 019 19.5z" clipRule="evenodd" /></> },
    { label: "Total Patients",  value: totalPatients,  color: "#808080", icon: <><path d="M4.5 6.375a4.125 4.125 0 118.25 0 4.125 4.125 0 01-8.25 0zM14.25 8.625a3.375 3.375 0 116.75 0 3.375 3.375 0 01-6.75 0zM1.5 19.125a7.125 7.125 0 0114.25 0v.003l-.001.119a.75.75 0 01-.363.63 13.067 13.067 0 01-6.761 1.873c-2.472 0-4.786-.684-6.76-1.873a.75.75 0 01-.364-.63l-.001-.122zM17.25 19.128l-.001.144a2.25 2.25 0 01-.233.96 10.088 10.088 0 005.06-1.01.75.75 0 00.42-.643 4.875 4.875 0 00-6.957-4.611 8.586 8.586 0 011.71 5.157v.003z" /></> },
  ];
...
<div
      ...
      <div className={`${CARD} overflow-hidden hidden sm:block`}>
        ...
        <div className={FOOT}>Showing {filteredPayers.length} of {payers.length} records</div>
      </div>
        ...
        {filteredPayers.map(p => (
          <div key={p.claimId} className={`${CARD} p-4 space-y-3`}>
            <div className="flex items-center justify-between gap-2">
              <span className="text-sm font-medium text-slate-800 tabular-nums">{p.claimId}</span>
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-sm font-medium dark:bg-slate-700 text-slate-800 dark:text-slate-300">{p.department}</span>
            </div>
            <div>
              <p className="text-sm font-medium text-slate-800 dark:text-slate-100">{p.patientName}</p>
              <p className="text-sm font-medium text-slate-800 dark:text-slate-400">{p.insuranceCompany}</p>
            </div>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div><p className="text-slate-800 font-medium mb-0.5">Phone</p><p className="text-slate-800 dark:text-slate-300">{p.phone}</p></div>
